...
public class UserController {
	...
	public String viewEnd(Model model, @RequestParam(name = "takeid", required = true) String takeid) {
		List<TakeAnswer> listTakeAnswers = takeAnswerRepository.findByTakeId(Long.parseLong(takeid));
		int score = 0;
		for (TakeAnswer c : listTakeAnswers) {
			if (c.getAnswer().isCorrect()) {
				score += c.getQuestion().getScore();
			}
		}
		
		model.addAttribute("id",listTakeAnswers.get(0).getTake().getQuiz().getId());
		model.addAttribute("score", score);
		Take  A= listTakeAnswers.get(0).getTake();
		A.setScore(score);
		takeRepository.save(A);
		return "end";
	}

	// Question page
	@GetMapping(value = { "/question" })
	public String viewQuestion(Model model, @RequestParam(name = "id") long id,
			@RequestParam(name = "questionid", defaultValue = "1") int questionid) {
...
//		model.addAttribute("list_quetionList",list_quetionList);
	...
	public String getTakeAnswer(Model model, @RequestParam(name = "takeid") long takeid,
			@RequestParam(name = "answer") long answerid, @RequestParam(name = "questionid") long questionid,
	...
	public String viewHighscore(Model model, @RequestParam(name = "quizid") long quizid) {
		List<Take> highscoreList = takeRepository.findByQuizIdAndSort(quizid);
		model.addAttribute("highscoreList",highscoreList);
		model.addAttribute("id",highscoreList.get(0).getQuiz().getId());
//		highscoreList.get(0).getUser().getUsername();
		return "highscores";
	}
}
